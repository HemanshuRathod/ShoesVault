...
const AdminOrders = () => {
  ...
  // Filter Logic including the new Address field
  const filteredOrders = orders.filter(o => {
    const sId = `#SV-${o._id.substring(0, 8)}`.toLowerCase();
    const sEmail = (o.user_email || "").toLowerCase();
    const sStatus = o.status.toLowerCase();
    const sAddress = (o.address || "").toLowerCase(); // New address search support
    const term = searchTerm.toLowerCase();

    return sId.includes(term) || sEmail.includes(term) || sStatus.includes(term) || sAddress.includes(term);
  });
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        ...
        <div className="input-group shadow-sm rounded-pill overflow-hidden border bg-white" style={{ maxWidth: '350px' }}>
          ...
          <input 
            type="text" 
            className="form-control border-0 py-2 shadow-none small" 
            placeholder="Search ID, Email, Address..." 
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
        </div>
                ...
                filteredOrders.map(o => (
                  <tr key={o._id}>
                    ...
                    <td className="text-start">
                      <div className="small fw-bold">{o.user_email || "N/A"}</div>
                    </td>
                    
                    {/* NEW ADDRESS COLUMN */}
                    <td className="text-start" style={{ maxWidth: '200px' }}>
                      <div className="small text-muted text-truncate" title={o.address}>
                        {o.address || <span className="text-danger italic">No Address Provided</span>}
                      </div>
                    </td>

                    <td className="fw-black text-success">₹{o.total_price}</td>
